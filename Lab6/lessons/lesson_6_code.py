...
class MultiArmedBandit():
	...
	def __init__( self, levers=10 ):
		self.levers = levers
		self.q_star = np.random.normal(0, 1, size=levers)
		self.sampling_variance = 1 # every action has the same variance
...
def banditAlgorithm( env, eps=0, maxiters=1000 ):
	...
	for _ in range(maxiters):
		...
		if np.random.choice([0, 1], 1, p=[1 - eps, eps]):
			action = np.random.randint(levers, size=1)
		else:
			action = np.argmax(Q)
